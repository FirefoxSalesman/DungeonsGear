...
import net.firefoxsalesman.dungeonsgear.network.PacketOffhandAttack;
...
public class CombatEventHandler {
	...
	public static void checkForOffhandAttack() {
		...
		if (Minecraft.getInstance().level != null && Minecraft.getInstance().screen == null
				&& !Minecraft.getInstance().isPaused() && player != null && !player.isBlocking()) {
			ItemStack offhand = player.getOffhandItem();
			if (offhand.getItem() instanceof IDualWieldWeapon
					&& ((IDualWieldWeapon<?>) offhand.getItem()).canAttack(player, offhand)) {
				float reach = (float) 3.0D;
				if (mc.player != null) {
					reach = (float) mc.player
							.getAttributeBaseValue(AttributeInit.ATTACK_REACH.get());
				}
				...
				if (rayTrace instanceof EntityHitResult) {
					...
					if (entityHit != player && entityHit != player.getVehicle()) {
						NetworkHandler.INSTANCE.sendToServer(
								new PacketOffhandAttack(entityHit.getId()));
					}
	...
	private static HitResult getEntityMouseOverExtended(float reach) {
		...
		if (viewEntity != null && mc.level != null) {
			...
			Vec3 attackVec = eyePos.add(lookVec.x * reachDistance, lookVec.y * reachDistance,
					lookVec.z * reachDistance);
			AABB axisAlignedBB = viewEntity.getBoundingBox().expandTowards(lookVec.scale(reachDistance))
					.inflate(1.0D, 1.0D, 1.0D);
			EntityHitResult entityRayTrace = ProjectileUtil.getEntityHitResult(viewEntity, eyePos,
					attackVec, axisAlignedBB,
					(entity) -> !entity.isSpectator() && entity.isPickable(), attackReach);
			if (entityRayTrace != null) {
				...
				if (hasExtendedReach && squareDistanceTo > (double) (reach * reach)) {
					result = BlockHitResult.miss(hitVec,
							Direction.getNearest(lookVec.x, lookVec.y, lookVec.z),
							new BlockPos((int) hitVec.x, (int) hitVec.y, (int) hitVec.z));
				} else if (squareDistanceTo < attackReach) {
					result = entityRayTrace;
				}
			} else {
				result = BlockHitResult.miss(attackVec,
						Direction.getNearest(lookVec.x, lookVec.y, lookVec.z),
						new BlockPos((int) attackVec.x, (int) attackVec.y, (int) attackVec.z));
			}
		}
