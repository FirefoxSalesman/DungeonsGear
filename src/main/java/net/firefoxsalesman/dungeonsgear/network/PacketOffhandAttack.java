package net.firefoxsalesman.dungeonsgear.network;
						...
						if (target != null) {
							...
							if (!offhand.isEmpty()) {
								if (offhand.getItem() instanceof IDualWieldWeapon) {
									...
									// This is done to mitigate the difference
									// between the render view entity's position
									// that is checked in the client to the server
									// player entity's position
									float renderViewEntityOffsetFromPlayerMitigator = 0.2F;
									reach += renderViewEntityOffsetFromPlayerMitigator;

									double distanceSquared = player
											.distanceToSqr(target);
									double reachSquared = reach * reach;

									if (reachSquared >= distanceSquared) {
										PlayerAttackHelper
												.attackTargetEntityWithCurrentOffhandItem(
														player,
														target);
									}

									PlayerAttackHelper.swingArm(player,
