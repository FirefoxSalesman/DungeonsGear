...
public class AddPotionLootFunction extends LootItemConditionalFunction {
	...
	public AddPotionLootFunction(LootItemCondition[] conditions,
			Collection<Pair<Potion, Float>> weightedPotionOptions, float totalWeight) {
		super(conditions);
		if (weightedPotionOptions.size() == 0) {
			isRegistryRandom = true;
			this.weightedPotionOptions = ForgeRegistries.POTIONS.getEntries().stream()
					.map(e -> Pair.of(e.getValue(), 1.0f)).collect(ImmutableList.toImmutableList());
		} else {
	...
	public LootItemFunctionType getType() {
		return ModLootFunctionTypes.ADD_POTION.get();
	}
	...
	public static class Builder extends LootItemConditionalFunction.Builder<AddPotionLootFunction.Builder> {
		...
		public AddPotionLootFunction build() {
			return new AddPotionLootFunction(this.getConditions(), options,
					(float) options.stream().mapToDouble(Pair::getSecond).sum());
		}
	}

	public static class Serializer extends LootItemConditionalFunction.Serializer<AddPotionLootFunction> {
		@Override
		public AddPotionLootFunction deserialize(JsonObject object,
				JsonDeserializationContext deserializationContext, LootItemCondition[] conditionsIn) {
			...
			if (object.has("potion")) {
				if (hasEntries) {
					throw new IllegalStateException(
							"Cannot specify both 'potion' and 'potions' at the same time!");
				}
		...
		@Override
		public void serialize(JsonObject json, AddPotionLootFunction lootFunction,
				JsonSerializationContext ctx) {
			super.serialize(json, lootFunction, ctx);

			if (lootFunction.weightedPotionOptions.size() > 0 && !lootFunction.isRegistryRandom) {
				boolean useStrings = lootFunction.weightedPotionOptions.stream()
						.allMatch(e -> e.getSecond() == 1.0f);
				...
				for (int i = 0; i < lootFunction.weightedPotionOptions.size(); i++) {
					...
					if (useStrings) {
						...
						entry.addProperty("potion", ForgeRegistries.POTIONS
								.getKey(pair.getFirst()).toString());
