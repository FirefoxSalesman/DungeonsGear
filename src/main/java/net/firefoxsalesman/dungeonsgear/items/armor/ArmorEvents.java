...
import net.firefoxsalesman.dungeonsgear.mixin.MobEffectInstanceAccessor;
...
import static net.firefoxsalesman.dungeonsgear.registry.ItemInit.CHAMPIONS_ARMOR;
import static net.firefoxsalesman.dungeonsgear.registry.ItemInit.HEROS_ARMOR;
...
public class ArmorEvents {
	...
	public static void onSpelunkerArmorEquipped(LivingEquipmentChangeEvent event) {
		if (event.getEntity() instanceof Player && event.getSlot() != EquipmentSlot.OFFHAND
	...
	public static void respawnPetBat(TickEvent.PlayerTickEvent event) {
		if (event.player.tickCount % 140 == 0)
			for (ItemStack i : event.player.getArmorSlots()) {
				if (i.getItem() instanceof PetBatArmorGear
						&& ((PetBatArmorGear) i.getItem()).doGivesYouAPetBat()) {
	...
	private static void reduceFreezingEffect(MobEffectEvent.Added event, MobEffectInstance effectInstance,
			ItemStack helmet, ItemStack chestplate) {
		float freezingResistance = helmet.getItem() instanceof FreezingResistanceArmorGear
				? (float) ((FreezingResistanceArmorGear) helmet.getItem()).getFreezingResistance()
				: 0;
		float freezingResistance2 = chestplate.getItem() instanceof FreezingResistanceArmorGear
				? (float) ((FreezingResistanceArmorGear) chestplate.getItem()).getFreezingResistance()
				: 0;
		...
		if (freezingMultiplier > 0) {
			if (event.getEffectInstance().getEffect() == MobEffects.MOVEMENT_SLOWDOWN
				...
				((MobEffectInstanceAccessor) effectInstance)
						.setDuration((int) (oldDuration * freezingMultiplier));
			}
		}
	}
