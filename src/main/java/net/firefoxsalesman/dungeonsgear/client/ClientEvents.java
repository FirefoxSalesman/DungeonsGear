...
@Mod.EventBusSubscriber(modid = DungeonsGear.MOD_ID, value = Dist.CLIENT)
		...
		if (cap.isUsingArtifact() && cap.getUsingArtifact().getItem() instanceof AbstractBeaconItem) {
			...
			if (event.getHand() == InteractionHand.MAIN_HAND) {
				...
				Minecraft.getInstance().getEntityRenderDispatcher().getItemInHandRenderer()
						.renderArmWithItem(player, partialTicks, f1, event.getHand(), 0.0f,
								cap.getUsingArtifact(), 0.0f, event.getPoseStack(),
