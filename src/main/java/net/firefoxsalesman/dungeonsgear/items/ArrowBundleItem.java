...
import net.firefoxsalesman.dungeonsgear.utilities.LootTableHelper;
...
public class ArrowBundleItem extends Item {
	...
	public InteractionResultHolder use(Level world, Player playerIn, InteractionHand handIn) {
		...
		if (!world.isClientSide) {
			...
			for (int i = 0; i < numberOfArrows; i++) {
				ItemStack itemStack = LootTableHelper.generateItemStack((ServerLevel) player.level(),
						player.blockPosition(),
						GeneralHelper.modLoc("items/arrow_bundle"), player.getRandom());
				ItemEntity arrow = new ItemEntity(world, player.getX(), player.getY(), player.getZ(),
						itemStack);
	...
	private int getNumberOfArrows(Player player, ItemStack bundleItemStack) {
		...
		return baseArrows + StreamSupport.stream(player.getArmorSlots().spliterator(), false)
				.map(this::arrowAmount).reduce(0, (a, b) -> a + b, Integer::sum);
	}

	private int arrowAmount(ItemStack stack) {
		return EnchantmentHelper.getItemEnchantmentLevel(EnchantmentInit.ARROW_HOARDER.get(), stack);
	}
